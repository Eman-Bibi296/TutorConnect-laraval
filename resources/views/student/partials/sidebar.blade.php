...
<div class="sidebar">
    ...
    <div class="profile-section">
        ...
        <h3 class="profile-name">{{ $student->name ?? 'Student' }}</h3>
        <p class="profile-email">{{ $student->email ?? 'student@email.com' }}</p>
    </div>
    ...
    <ul class="sidebar-menu">
        ...
        <!-- My Requests -->
        <li>
            <a href="/student/my-requests" class="{{ request()->is('student/my-requests*') ? 'active' : '' }}">
              <img src="{{ asset('images/Myrequest.png') }}" alt="My Requests" class="icon">
                My Requests
                @if(isset($unreadRequestChanges) && $unreadRequestChanges > 0)
                    <span class="badge badge-request">{{ $newRequestChanges }}</span>
                @endif
            </a>
        </li>
        ...
        <!-- Messages -->
        <li>
            <a href="/student/messages" class="{{ request()->is('student/messages*') ? 'active' : '' }}">
                 <img src="{{ asset('images/messages.png') }}" alt="Messages" class="icon">
                Messages
                @if($newMessagesCount > 0)
                    <span class="badge badge-message">{{ $newMessagesCount }}</span>
                @endif
            </a>
        </li>
        ...
        <!-- My Bookings -->
        <li>
            <a href="/student/my-bookings" class="{{ request()->is('student/my-bookings*') ? 'active' : '' }}">
                 <img src="{{ asset('images/bookingicon.png') }}" alt="My Bookings" class="icon">
                 My Booking
                @if(isset($unreadBookings) && $unreadBookings > 0)
        ...
        <!-- Study Materials -->
        <li>
            <a href="/student/study-materials" class="{{ request()->is('student/study-materials*') ? 'active' : '' }}">
               <img src="{{ asset('images/studymaterial.png') }}" alt="Study Materials" class="icon">
                Study Materials
                 @if(isset($unreadMaterials) && $unreadMaterials > 0)
                <span class="badge badge-materials">{{ $unreadMaterials }}</span>
            @endif
            </a>
        </li>
        ...
        <!-- Reviews -->
        <li>
            <a href="/student/reviews" class="{{ request()->is('student/reviews*') ? 'active' : '' }}">
