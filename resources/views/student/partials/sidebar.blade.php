...
<div class="sidebar">
    ...
    <img src="{{ $studentAvatar ? asset($studentAvatar) : 'https://ui-avatars.com/api/?name=' . urlencode($studentUser->name ?? 'Student') . '&background=ECFDF5&color=059669' }}" alt="Student Avatar" class="student-avatar">
        <h4>{{ $studentUser->name ?? 'Student' }}</h4>
        <p>{{ $studentUser->email ?? 'student@tutorconnect.com' }}</p>
    </div>
    ...
    <ul class="sidebar-menu">
        ...
        <li>
            <a href="/student/my-requests" class="{{ request()->is('student/my-requests*') ? 'active' : '' }}">
                ...
                @if($unreadReqCount > 0)
                    <span class="badge-count badge-req">{{ $unreadReqCount }}</span>
                @endif
            </a>
        </li>
        <li>
            <a href="/student/messages" class="{{ request()->is('student/messages*') ? 'active' : '' }}">
                ...
                <span>Messages</span>
                @if($unreadMsgCount > 0)
                    <span class="badge-count badge-msg">{{ $unreadMsgCount }}</span>
                @endif
            </a>
        </li>
        <li>
            <a href="/student/my-bookings" class="{{ request()->is('student/my-bookings*') ? 'active' : '' }}">
                ...
                @if($unreadBkCount > 0)
                    <span class="badge-count badge-bk">{{ $unreadBkCount }}</span>
                @endif
            </a>
        </li>
        <li>
            <a href="/student/study-materials" class="{{ request()->is('student/study-materials*') ? 'active' : '' }}">
                ...
                @if($unreadMatCount > 0)
                    <span class="badge-count badge-mat">{{ $unreadMatCount }}</span>
                @endif
            </a>
        </li>
        <li>
